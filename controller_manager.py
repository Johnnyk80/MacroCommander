...
BUTTON_MAP = {
    ...
    0x0040: "LS", 0x0080: "RS", 0x0001: "DPad Up", 0x0002: "DPad Down",
    0x0004: "DPad Left", 0x0008: "DPad Right",
}

GENERIC_BUTTON_MAP = {0: "A", 1: "B", 2: "X", 3: "Y", 4: "LB", 5: "RB", 6: "Back", 7: "Start", 8: "LS", 9: "RS"}

INPUTS_BUTTON_MAP = {
    "BTN_SOUTH": "A", "BTN_A": "A", "BTN_EAST": "B", "BTN_B": "B",
    "BTN_WEST": "X", "BTN_X": "X", "BTN_NORTH": "Y", "BTN_Y": "Y",
    "BTN_TL": "LB", "BTN_TR": "RB", "BTN_SELECT": "Back", "BTN_MODE": "Back",
    "BTN_START": "Start", "BTN_THUMBL": "LS", "BTN_THUMBR": "RS",
    "BTN_DPAD_UP": "DPad Up", "BTN_DPAD_DOWN": "DPad Down", "BTN_DPAD_LEFT": "DPad Left", "BTN_DPAD_RIGHT": "DPad Right",
}

WINMM_BUTTON_MAP = {0: "A", 1: "B", 2: "X", 3: "Y", 4: "LB", 5: "RB", 6: "Back", 7: "Start", 8: "LS", 9: "RS"}
...
class ControllerManager:
    def __init__(self, macro_engine, max_controllers=4, poll_hz=100):
        ...
        for cid in self._known_ids:
            ...
            self.names[cid] = f"XInput Controller {cid.split(':')[1]}"
    ...
    def _xid(self, idx): return f"xinput:{int(idx)}"
    def _pid(self, idx): return f"pygame:{int(idx)}"
    def _iid(self, idx): return f"inputs:{int(idx)}"
    def _wid(self, idx): return f"winmm:{int(idx)}"
    ...
    def _start_inputs_loop_if_available(self):
        if INPUTS is None or not callable(getattr(INPUTS, "get_gamepad", None)):
            return
        threading.Thread(target=self._inputs_global_loop, daemon=True).start()
    ...
    def _buttons_to_names(self, wbuttons):
        out = [name for mask, name in BUTTON_MAP.items() if (wbuttons & mask)]
        out.sort()
        return tuple(out)

    def _norm_axis(self, value):
        try:
            return int(max(-32768, min(32767, float(value) * 32767.0)))
        except Exception:
            return 0

    def _norm_trigger(self, value):
        try:
            v = float(value)
        except Exception:
            return 0
    ...
    def _norm_u16_to_s16(self, v):
        try:
            vv = int(v)
        except Exception:
            vv = 32767
        return int(max(-32768, min(32767, ((vv / 65535.0) * 65535.0) - 32768.0)))

    def _norm_inputs_trigger(self, v):
        try:
            vv = int(v)
        except Exception:
            vv = 0
        if vv > 255:
            vv = int((vv / 1023.0) * 255.0) if vv <= 1023 else int((vv / 65535.0) * 255.0)
        return int(max(0, min(255, vv)))
        ...
        for idx in range(count):
            ...
            try:
                ...
                for h in range(int(joy.get_numhats())):
                    hx, hy = joy.get_hat(h)
                    if hy > 0: pressed.add("DPad Up")
                    elif hy < 0: pressed.add("DPad Down")
                    if hx < 0: pressed.add("DPad Left")
                    elif hx > 0: pressed.add("DPad Right")
                self.pressed[cid] = tuple(sorted(pressed))
                axes = [joy.get_axis(a) for a in range(int(joy.get_numaxes()))]
                self.latest[cid] = GenericGamepadState(
                    lt=self._norm_trigger(axes[4]) if len(axes) > 4 else 0,
                    rt=self._norm_trigger(axes[5]) if len(axes) > 5 else 0,
                    lx=self._norm_axis(axes[0]) if len(axes) > 0 else 0,
                    ly=self._norm_axis(axes[1]) if len(axes) > 1 else 0,
                    rx=self._norm_axis(axes[2]) if len(axes) > 2 else 0,
                    ry=self._norm_axis(axes[3]) if len(axes) > 3 else 0,
                )
            except Exception:
                self.connected[cid] = False
                self.latest[cid] = None
    ...
    def _poll_winmm(self):
        ...
        for idx in range(count):
            ...
            res = WINMM.joyGetPosEx(idx, ctypes.byref(info))
            ...
            self.connected[cid] = True
            self.names[cid] = f"DirectInput/RawInput Controller {idx} (winmm)"

            pressed = set()
            buttons = int(info.dwButtons)
            for bit, name in WINMM_BUTTON_MAP.items():
                if buttons & (1 << bit):
                    pressed.add(name)

            pov = int(info.dwPOV)
            if pov != 0xFFFF:
                if pov in (0, 4500, 31500):
                    pressed.add("DPad Up")
                if pov in (4500, 9000, 13500):
                    pressed.add("DPad Right")
                if pov in (13500, 18000, 22500):
                    pressed.add("DPad Down")
                if pov in (22500, 27000, 31500):
                    pressed.add("DPad Left")

            self.pressed[cid] = tuple(sorted(pressed))
            self.latest[cid] = GenericGamepadState(
                lt=max(0, min(255, int((int(info.dwZpos) / 65535.0) * 255.0))),
                rt=max(0, min(255, int((int(info.dwRpos) / 65535.0) * 255.0))),
                lx=self._norm_u16_to_s16(info.dwXpos),
                ly=self._norm_u16_to_s16(info.dwYpos),
                rx=self._norm_u16_to_s16(info.dwUpos),
                ry=self._norm_u16_to_s16(info.dwVpos),
            )

        for cid in [x for x in self._known_ids if x.startswith("winmm:")]:
            if cid not in seen:
                self.connected[cid] = False
                self.latest[cid] = None
                self.pressed[cid] = tuple()
        ...
        while True:
            ...
            with self._inputs_lock:
                ...
                if cid not in self._inputs_axes:
                    self._inputs_axes[cid] = {"ABS_X": 0, "ABS_Y": 0, "ABS_RX": 0, "ABS_RY": 0, "ABS_Z": 0, "ABS_RZ": 0}
                pressed = set(self.pressed.get(cid, tuple()))
                axis = self._inputs_axes[cid]

                for ev in events:
                    dev_name = str(getattr(getattr(ev, "device", None), "name", "") or "Bluetooth/Generic Controller")
                    self.names[cid] = f"{dev_name} (inputs)"
                    code = str(getattr(ev, "code", ""))
                    state = int(getattr(ev, "state", 0))
                    if code in INPUTS_BUTTON_MAP:
                        if state:
                            pressed.add(INPUTS_BUTTON_MAP[code])
                        else:
                            pressed.discard(INPUTS_BUTTON_MAP[code])
                    elif code == "ABS_HAT0X":
                        pressed.discard("DPad Left")
                        pressed.discard("DPad Right")
                        if state < 0: pressed.add("DPad Left")
                        elif state > 0: pressed.add("DPad Right")
                    elif code == "ABS_HAT0Y":
                        pressed.discard("DPad Up")
                        pressed.discard("DPad Down")
                        if state < 0: pressed.add("DPad Up")
                        elif state > 0: pressed.add("DPad Down")
                    elif code in axis:
                        axis[code] = state

                self.pressed[cid] = tuple(sorted(pressed))
                self.latest[cid] = GenericGamepadState(
                    lt=self._norm_inputs_trigger(axis.get("ABS_Z", 0)),
                    rt=self._norm_inputs_trigger(axis.get("ABS_RZ", 0)),
                    lx=self._norm_u16_to_s16(axis.get("ABS_X", 0) + 32768),
                    ly=self._norm_u16_to_s16(axis.get("ABS_Y", 0) + 32768),
                    rx=self._norm_u16_to_s16(axis.get("ABS_RX", 0) + 32768),
                    ry=self._norm_u16_to_s16(axis.get("ABS_RY", 0) + 32768),
                )
    ...
    def get_hid_gamepad_devices(self):
        ...
        try:
            for dev in HID.enumerate():
                if int(dev.get("usage_page", -1)) == 0x01 and int(dev.get("usage", -1)) in (0x04, 0x05):
                    out.append(dev)
        except Exception:
            return []
        return out
    ...
    def _set_vibration(self, xinput_id, left_speed, right_speed):
        if XINPUT is None:
            return 1
        vib = XINPUT_VIBRATION(int(max(0, min(65535, left_speed))), int(max(0, min(65535, right_speed))))
    ...
    def vibrate(self, controller_id, left=32000, right=32000, duration_ms=120):
        ...
        if not cid.startswith("xinput:") or not self.connected.get(cid, False):
            return
    ...
    def run(self):
        while True:
            for idx in range(self.max_controllers):
                ...
                self.pressed[cid] = self._buttons_to_names(gp.wButtons)
                if self.macro_engine is not None:
                    self.macro_engine.check_combo(cid, self.pressed[cid])

            self._poll_pygame()
            self._poll_winmm()

            for cid in [x for x in self._known_ids if x.startswith("pygame:") or x.startswith("inputs:") or x.startswith("winmm:")]:
                if self.macro_engine is not None:
